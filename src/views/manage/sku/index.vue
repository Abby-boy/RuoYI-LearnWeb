  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="skuList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="商品价格" align="center">
          <template #default="scope">
                  <el-tag>{{ (scope.row.price / 100).toFixed(2) }} 元</el-tag>
          </template>
      </el-table-column>
    ...
    <!-- 导入文件对话框 -->
    <!-- 导入文件对话框 -->
    <el-dialog v-model="excelOpen" title="数据导入" width="400px" append-to-body>
      <el-upload
        class="upload-demo"
        ref="uploadRef"
        ...
        <template #tip>
          <div class="el-upload__tip">只能上传xls,xlsx文件，且不超过1MB</div>
        </template>
      </el-upload> 
    </el-dialog>
  </div>
...
// 上传成功回调
function handleExcelSuccess(res, file) {
  if (res.code === 200) {
...
function handleExport() {
  proxy.download('manage/sku/export', {
    ...queryParams.value,
    ids: ids.value.join(',')    // 将选中的商品id 列表转为字符串
